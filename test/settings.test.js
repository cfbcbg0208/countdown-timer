...
test('progressShow: 기본 바·도넛 on·독립% off, 각 파트 불리언(누락 → true)', () => {
  const st = fakeStorage();
  assert.deepEqual(load(st).progressShow, { bar: true, pie: true, percent: false });
  assert.deepEqual(update(st, { progressShow: { bar: false } }).progressShow, {
    bar: false,
    pie: true,
    percent: true, // 누락은 true(저장 객체가 명시하지 않은 경우)
  });
});
